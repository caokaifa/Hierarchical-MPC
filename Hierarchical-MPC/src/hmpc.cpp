...
void HMPC::getParameters(ros::NodeHandle &nh) {
    ...
    nh.getParam("qf_x",qf_x);
    nh.getParam("qf_y",qf_y);
    nh.getParam("qf_yaw",qf_yaw);
...
void HMPC::odom_callback(const nav_msgs::Odometry::ConstPtr &odom_msg){
   ...
   // std::cout<<"----q_tf----"<<odom_msg->pose.pose.orientation<<" "<<q_tf<<std::endl;
    ...
    double last_distance = sqrt(pow(track_.centerline.back().x-car_pos_[0], 2)+pow(track_.centerline.back().y-car_pos_[1], 2));
    double theta_end = track_.findTheta(car_pos_[0], car_pos_[1], 0, true);
    std::cout<<"----distance----="<<last_distance<<" "<<theta_end<<std::endl;
    if((last_distance<2.5) && (theta_end>55))
    {
        ...
        last_speed_cmd_ = 0;
        std::cout<<"----the goal is arrived-----"<<std::endl;
...
void HMPC::select_trajectory(){
   ...
   visualize_trajectories(low_ind, high_ind);
    ...
    bool trajectory_query_falg_=false;
        ...
        for (int j = low_ind; j <= high_ind; j++) {
           ...
           //找出最大前进距离轨迹
            if (theta>max_theta){
                ...
                max_theta = theta;
                trajectory_query_falg_=true;//找出最大前进距离轨迹
            }
            // if(trajectory_query_falg_)
            // {
            //     break;
            // }
        }
...
void HMPC::execute_MPC(){
    ...
    // std::cout<<"----nx---="<<nx<<" "<<nu<<" "<<N<<" "<<(N+1)*nx+ (N+1) +(N+1)*nu + N;
    /** MPC variables: z = [x0, ..., xN, u0, ..., uN]**/
    ...
    SparseMatrix<double> constraintMatrix((N+1)*nx+ (N+1) +(N+1)*nu + N, (N+1)*(nx+nu));//(N+1)*nx为迭代状态量 (N+1)为边界 (N+1)*nu为控制输入 N为加速度 

    VectorXd gradient((N+1)*(nx+nu));
    gradient.setZero();

    VectorXd lower((N+1)*nx+ (N+1) +(N+1)*nu + N);
    VectorXd upper((N+1)*nx+ (N+1) +(N+1)*nu + N);
    ...
    for (int i=0; i<N+1; i++){        //0 to N
        ...
        if (i>0) {
            ...
            else
            {
                for (int row = 0; row < nx; row++) 
                {
                    HessianMatrix.insert(i*nx + row, i*nx + row) = Qf(row, row);
                }
        ...
        if (i<N){
            ...
            upper.segment<nx>((i+1)*nx) = -hd;  //等式约束x(k+1)-Adx(k)-Bd(k)=-hd
        }
        ...
        constraintMatrix.insert((N+1)*nx+i, i*nx) = -dy_dtheta;      // a*x
        constraintMatrix.insert((N+1)*nx+i, i*nx+1) = dx_dtheta;     // b*y
        ...
        if(i<N){
            constraintMatrix.insert((N+1)*nx+ (N+1) +(N+1)*nu +i, (N+1)*nx+i*nu) = -1;
            constraintMatrix.insert((N+1)*nx+ (N+1) +(N+1)*nu +i, (N+1)*nx+(i+1)*nu) = 1;
            lower((N+1)*nx+ (N+1) +(N+1)*nu  +i) = -OsqpEigen::INFTY;
            upper((N+1)*nx+ (N+1) +(N+1)*nu  +i) = ACCELERATION_MAX *Ts;//加速度约束
        }       
    ...
    solver.data()->setNumberOfConstraints((N+1)*nx+ (N+1) +(N+1)*nu + N);
